...
fn update_proc_info(
    ...
    if proc_.name.is_empty() || cmd_needs_update {
        let cmd = unsafe { from_cstr_array(ffi::kvm_getargv2(kd.as_ptr(), kproc, 0) as _) };
    ...
    {
        proc_.environ = unsafe { from_cstr_array(ffi::kvm_getenvv2(kd.as_ptr(), kproc, 0) as _) };
    }

    // We now get the values needed for both new and existing process.
    if refresh_kind.cpu() {
        proc_.cpu_usage = (100 * kproc.p_pctcpu) as f32 / fscale;
    }
    ...
    proc_.status = match kproc.p_realstat {
        ...
        ffi::SACTIVE => {
            get_active_status(kd, kproc).unwrap_or(ProcessStatus::Unknown(kproc.p_realstat as _))
        }
    ...
    if refresh_kind.memory() {
        ...
        proc_.memory = (kproc.p_vm_rssize as u64).saturating_mul(page_size as _);
    }
...
pub(crate) unsafe fn get_process_data(
    ...
    if let Some(proc_) = unsafe { (*wrap.0.get()).get_mut(&Pid(kproc.p_pid)) } {
        ...
        if proc_.start_time == start_time {
            update_proc_info(kproc, page_size, fscale, now, refresh_kind, kd, proc_);
    ...
    update_proc_info(kproc, page_size, fscale, now, refresh_kind, kd, &mut inner);
